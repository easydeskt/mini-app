...
export const ColorPicker = ({
  ...
  // Keep latest onChange in a ref so the effect below doesn't need it as a dep
  const onChangeRef = useRef(onChange);
  useEffect(() => { onChangeRef.current = onChange; });

  // Fix: correctly convert value → HSL when the controlled value changes
  useEffect(() => {
    if (value) {
      const c = Color(value);
      setHue(c.hue());
  ...
  // Fix: removed onChange from deps — was causing an infinite re-render loop
  useEffect(() => {
...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    // Derive initial thumb position from context saturation/lightness
    ...
    const [positionY, setPositionY] = useState(() => {
      ...
      return Math.max(0, Math.min(1, topL > 0 ? 1 - lightness / topL : 0));
    });
...
export const ColorPickerOutput = ({
  ...
  return (
    <Select onValueChange={setMode} value={mode}>
      <SelectTrigger className="w-20 shrink-0 text-xs" size="sm" {...props}>
...
export const ColorPickerFormat = ({
  ...
  if (mode === "hex") {
    ...
    return (
      <div
        className={cn(
          "-space-x-px relative flex h-8 w-full items-center rounded-md shadow-sm",
          className
        )}
        {...props}
      >
  ...
  if (mode === "rgb") {
    ...
    return (
      <div
        className={cn(
          "-space-x-px flex h-8 items-center rounded-md shadow-sm",
          className
        )}
        {...props}
      >
...
export type ColorPickerPreviewProps = HTMLAttributes<HTMLSpanElement>;
